...
model {
  ...
  // priors
  target += normal_lpdf(beta_hail[1] | -7.0, 1.0 / 6.0);
  target += normal_lpdf(beta_hail[2] | 0.0932, 1.0 / 30.0);
  target += normal_lpdf(beta_report[1] | -2, 11.0 / 30.0);
  target += normal_lpdf(beta_report[2] | 1.0, 1.0 / 12.0);
}
