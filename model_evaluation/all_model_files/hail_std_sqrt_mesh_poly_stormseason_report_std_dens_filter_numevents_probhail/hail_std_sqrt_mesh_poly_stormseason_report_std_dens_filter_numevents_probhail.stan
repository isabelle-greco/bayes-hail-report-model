...
data {
  ...
  vector[8] beta_hail_lower_bounds;
...
transformed data {
  ...
  matrix[N_train, 8] X_mesh_train_std;
  matrix[N_train, 2] X_report_train_std;
  matrix[N_test, 8] X_mesh_test_std;
...
// parameters
parameters {
  vector<lower=beta_hail_lower_bounds>[8] beta_hail_raw;
...
transformed parameters {
  ...
  beta_hail[3:8] = beta_hail_raw[3:8];
  ...
  {
    vector[N_train] k_vector = X_mesh_train[, 3:8] * beta_hail[3:8];
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[3:8] | 0.0, 1.0 / 3.0);
