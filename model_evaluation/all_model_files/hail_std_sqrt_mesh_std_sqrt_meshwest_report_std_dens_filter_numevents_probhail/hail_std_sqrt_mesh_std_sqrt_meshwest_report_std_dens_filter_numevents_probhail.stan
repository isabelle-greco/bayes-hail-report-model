...
transformed data {
  ...
  matrix[N_train, 3] X_mesh_train_std;
  matrix[N_train, 2] X_report_train_std;
  matrix[N_test, 3] X_mesh_test_std;
  ...
  X_mesh_train_std[, 3] = (X_mesh_train[, 3] - mesh_train_untrans_mean) / mesh_train_untrans_sd;
  X_mesh_test_std[, 1] = X_mesh_test[, 1];
  X_mesh_test_std[, 2] = (X_mesh_test[, 2] - mesh_train_mean) / mesh_train_sd;
  X_mesh_test_std[, 3] = (X_mesh_test[, 3] - mesh_train_untrans_mean) / mesh_train_untrans_sd;
...
transformed parameters {
  ...
  beta_hail[1] = beta_hail_raw[1] - (beta_hail_raw[2] * (mesh_train_mean / mesh_train_sd)) - (beta_hail_raw[3] * (mesh_train_untrans_mean / mesh_train_untrans_sd));
  beta_hail[2] = beta_hail_raw[2] / mesh_train_sd;
  beta_hail[3] = beta_hail_raw[3] / mesh_train_untrans_sd;
  ...
  {
    vector[N_train] k_vector = X_mesh_train[, 3] * beta_hail[3];
    mesh_at_prob_0p5 = square(-(beta_hail[1] + mean(k_vector)) / beta_hail[2]);
  }
}
...
model {
  ...
  target += normal_lpdf(expected_total_hail_events_train | 1250.0 * N_train / (N_train + N_test), 250.0 * N_train / (N_train + N_test));
}
