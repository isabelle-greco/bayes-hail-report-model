...
transformed data {
  // since we standardised after YJ which changes each iteration only
  // transforming the mesh data here 
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[2] | 0.5, 0.5);
  target += normal_lpdf(beta_report_raw[1] | -2.0, 4.0);
  target += normal_lpdf(beta_report_raw[2] | 0.5, 0.5);
