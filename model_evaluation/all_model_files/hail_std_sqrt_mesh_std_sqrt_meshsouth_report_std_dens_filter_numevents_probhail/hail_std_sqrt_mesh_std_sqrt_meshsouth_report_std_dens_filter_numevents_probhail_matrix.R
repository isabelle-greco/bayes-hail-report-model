...
model_data <- read_csv(args[2], col_types = "nnTnnnnnnncnffffffn") %>%
  mutate(mesh_south = replace_na(mesh_south, 0)) # na's to zero


# create matrices
# matrix for hail
model_frame_hail <- model_data %>% 
  model.frame(report ~ sqrt(mesh) + sqrt(mesh_south) + folds, data = .)
model_matrix_hail <- model.matrix(report ~ sqrt(mesh) + sqrt(mesh_south) + folds, data = model_frame_hail)
