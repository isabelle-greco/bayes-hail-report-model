...
data {
  int<lower=0> N_train;
  int<lower=0> N_test;
  matrix[N_train, 3] X_mesh_train;
  matrix[N_test, 3] X_mesh_test;
  ...
  vector[5] beta_hail_lower_bounds;
  vector[3] beta_report_lower_bounds;
}
// parameters
parameters {
  vector<lower=beta_hail_lower_bounds>[5] beta_hail_raw;
...
transformed parameters {
  ...
  matrix[N_train, 3] X_mesh_train_std_trans;
  matrix[N_test, 3] X_mesh_test_std_trans;
  ...
  vector[N_train] mesh_train_yj_hail;
  vector[N_test] mesh_test_yj_hail;
  vector[N_train] mesh_train_yj_hail2;
  vector[N_test] mesh_test_yj_hail2;
  vector[N_train] dens_train_yj;
  vector[N_test] dens_test_yj;

  // applying transform
  for (i in 1:N_train) {
    mesh_train_yj_hail[i] = yeo_johnson(X_mesh_train[i, 2], beta_hail_raw[3]);
    dens_train_yj[i] = yeo_johnson(X_report_train[i, 2], beta_report_raw[3]);
    mesh_train_yj_hail2[i] = yeo_johnson(X_mesh_train[i, 3], beta_hail_raw[5]);
  }
  for (i in 1:N_test) {
    mesh_test_yj_hail[i] = yeo_johnson(X_mesh_test[i, 2], beta_hail_raw[3]);
    dens_test_yj[i] = yeo_johnson(X_report_test[i, 2], beta_report_raw[3]);
    mesh_test_yj_hail2[i] = yeo_johnson(X_mesh_test[i, 3], beta_hail_raw[5]);
  }
  
  // calculating statistics
  real mesh_train_yj_hail_mean = mean(mesh_train_yj_hail);
  real mesh_train_yj_hail_sd = sd(mesh_train_yj_hail);
  real dens_train_yj_mean = mean(dens_train_yj);
  real dens_train_yj_sd = sd(dens_train_yj);  
  real mesh_train_yj_hail2_mean = mean(mesh_train_yj_hail2);
  real mesh_train_yj_hail2_sd = sd(mesh_train_yj_hail2);

  // making transformed and normalised matrics
  X_mesh_train_std_trans[, 1] = X_mesh_train[, 1];
  X_mesh_train_std_trans[, 2] = (mesh_train_yj_hail - mesh_train_yj_hail_mean) / mesh_train_yj_hail_sd;
  X_mesh_train_std_trans[, 3] = (mesh_train_yj_hail2 - mesh_train_yj_hail2_mean) / mesh_train_yj_hail2_sd;

  X_mesh_test_std_trans[, 1] = X_mesh_test[, 1];
  X_mesh_test_std_trans[, 2] = (mesh_test_yj_hail - mesh_train_yj_hail_mean) / mesh_train_yj_hail_sd;
  X_mesh_test_std_trans[, 3] = (mesh_test_yj_hail2 - mesh_train_yj_hail2_mean) / mesh_train_yj_hail2_sd;
  ...
  X_report_test_std_trans[, 2] = (dens_test_yj - dens_train_yj_mean) / dens_train_yj_sd;
  {
    ...
    vector[3] beta_hail_raw_mult = append_row(beta_hail_raw[1:2], beta_hail_raw[4]);
    vector[N_train] prob_hail_train = inv_logit(X_mesh_train_std_trans * beta_hail_raw_mult);
  ...
  beta_hail[1] = beta_hail_raw[1] - (beta_hail_raw[2] * (mesh_train_yj_hail_mean / mesh_train_yj_hail_sd)) - (beta_hail_raw[4] * (mesh_train_yj_hail2_mean / mesh_train_yj_hail2_sd));
  beta_hail[2] = beta_hail_raw[2] / mesh_train_yj_hail_sd;
  beta_hail[3] = beta_hail_raw[3];
  beta_hail[4] = beta_hail_raw[4] / mesh_train_yj_hail2_sd;
  beta_hail[5] = beta_hail_raw[5];
}
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[3] | 1.0, 2.0 / 3.0);
  target += normal_lpdf(beta_hail_raw[4] | 0.0, 1.0);
  target += normal_lpdf(beta_hail_raw[5] | 1.0, 2.0 / 3.0);
  target += normal_lpdf(beta_report_raw[1] | -2.0, 4.0);
  target += gamma_lpdf(beta_report_raw[2] | 6.0, 3.0);
  target += normal_lpdf(beta_report_raw[3] | 1.0, 2.0 / 3.0);
}
...
generated quantities {
  ...
  {
    vector[3] beta_hail_raw_mult = append_row(beta_hail_raw[1:2], beta_hail_raw[4]);
    vector[N_test] prob_hail_test = inv_logit(X_mesh_test_std_trans * beta_hail_raw_mult);
