...
transformed parameters {
  ...
  mesh_at_50_prob = (logit(0.5) - beta_hail[1]) / beta_hail[2];
}
...
model {
  ...
  // priors
  target += normal_lpdf(beta_hail_raw[1] | 0.0, 1.0);
  target += normal_lpdf(beta_hail_raw[2] | 0.0, 1.0);
  target += normal_lpdf(beta_report_raw[1] | 0.0, 1.0);
  target += normal_lpdf(beta_report_raw[2] | 0.0, 1.0);
  // stronger prior information on number of hail events
  target += normal_lpdf(expected_total_hail_events_train | 196 * N_train / (N_train + N_test), 40.0);
  // stronger prior information on mesh leading to 0.5 prob of hail
  target += skew_normal_lpdf(mesh_at_50_prob | 25.0, 8.0, 4.0);
}
