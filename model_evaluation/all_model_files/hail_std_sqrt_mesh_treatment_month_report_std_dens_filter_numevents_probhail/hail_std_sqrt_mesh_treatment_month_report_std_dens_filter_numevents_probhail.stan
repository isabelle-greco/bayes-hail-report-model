...
transformed data {
  ...
  matrix[N_train, 9] X_mesh_train_std;
  matrix[N_train, 2] X_report_train_std;
  matrix[N_test, 9] X_mesh_test_std;
...
transformed parameters {
  ...
  {
    vector[N_train] k_vector = X_mesh_train[, 3:9] * beta_hail[3:9];
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[3:9] | 0.0, 1.0 / 3.0);
