...
model_frame_report <- model_data %>% 
  ...
  mutate(report = case_when(report == 0 ~ 0,
                            report == 1 ~ 1)) %>%  
  model.frame(~ pop_dens + mesh_max + report + folds, data = .)
model_matrix_report <- model.matrix(~ pop_dens + mesh_max + report + folds, data = model_frame_report)
...
beta_report_lower_bounds <- c(-Inf, 0.0, -Inf, -Inf, -Inf)
