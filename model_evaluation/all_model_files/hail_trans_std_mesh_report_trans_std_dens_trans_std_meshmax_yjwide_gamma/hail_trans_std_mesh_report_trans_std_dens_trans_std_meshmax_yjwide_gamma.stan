...
transformed parameters {
  ...
  for (i in 1:N_train) {
    ...
    mesh_train_yj_report[i] = yeo_johnson(X_report_train[i, 3], beta_report_raw[5]);
  }
  for (i in 1:N_test) {
    ...
    mesh_test_yj_report[i] = yeo_johnson(X_report_test[i, 3], beta_report_raw[5]);
  }
  ...
  X_report_train_std_trans[, 2] = (dens_train_yj - dens_train_yj_mean) / dens_train_yj_sd;
   X_report_train_std_trans[, 3] = (mesh_train_yj_report - mesh_train_yj_report_mean) / mesh_train_yj_report_sd;

  X_report_test_std_trans[, 1] = X_report_test[, 1];
  X_report_test_std_trans[, 2] = (dens_test_yj - dens_train_yj_mean) / dens_train_yj_sd;
  X_report_test_std_trans[, 3] = (mesh_test_yj_report - mesh_train_yj_report_mean) / mesh_train_yj_report_sd;
  {
    // can't have constraints in these local blocks
    // note the yj parameter is in the last entry of beta_hail
    vector[3] beta_report_raw_mult = append_row(beta_report_raw[1:2], beta_report_raw[4]);
    vector[N_train] prob_hail_train = inv_logit(X_mesh_train_std_trans * beta_hail_raw[1:2]);
    vector[N_train] prob_report_given_hail_train = inv_logit(X_report_train_std_trans * beta_report_raw_mult);
    prob_report_train = prob_hail_train .* prob_report_given_hail_train;
  }

  // unstandardised parameters  
  beta_report[1] = beta_report_raw[1] - (beta_report_raw[2] * (dens_train_yj_mean / dens_train_yj_sd)) - (beta_report_raw[4] * (mesh_train_yj_report_mean / mesh_train_yj_report_sd));
  beta_report[2] = beta_report_raw[2] / dens_train_yj_sd;
  beta_report[3] = beta_report_raw[3];
  beta_report[4] = beta_report_raw[4] / mesh_train_yj_report_sd;
  beta_report[5] = beta_report_raw[5];
  beta_hail[1] = beta_hail_raw[1] - (beta_hail_raw[2] * (mesh_train_yj_hail_mean / mesh_train_yj_hail_sd));
...
model {
  ...
  target += normal_lpdf(beta_report_raw[4] | 0.0, 1.0);
  target += normal_lpdf(beta_report_raw[5] | 1.0, 2.0 / 3.0);
}
...
generated quantities {
  ...
  // probability of report 
  {
    ...
    vector[N_test] prob_report_given_hail_test = inv_logit(X_report_test_std_trans * beta_report_raw_mult);
