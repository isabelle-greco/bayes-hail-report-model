...
model_data <- read_csv(args[2], col_types = "nnTnnnnnnncnffffffn") %>%
  # storm season as ordered factor
  mutate(storm_season = factor(storm_season, levels = c("2009/10", "2010/11", "2011/12", "2012/13", "2013/14", "2014/15", "2015/16"), ordered = TRUE))


# create matrices
# matrix for hail
model_frame_hail <- model_data %>% 
  model.frame(report ~ mesh + storm_season + folds, data = .)
model_matrix_hail <- model.matrix(report ~ mesh + storm_season + folds, data = model_frame_hail,
				  contrasts = list(storm_season = "contr.treatment"))
...
beta_hail_lower_bounds <- c(-Inf, 0.0, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf)
