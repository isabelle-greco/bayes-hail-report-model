...
model_frame_hail <- model_data %>% 
  model.frame(report ~ sqrt(mesh) + storm_season + folds, data = .)
model_matrix_hail <- model.matrix(report ~ sqrt(mesh) + storm_season + folds, data = model_frame_hail,
...
beta_hail_lower_bounds <- c(-Inf, 0.0, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf)
