...
model_data <- read_csv(args[2], col_types = "nnTnnnnnnncnffffffn") %>%
  mutate(mesh_west = replace_na(mesh_west, 0))


# create matrices
# matrix for hail
model_frame_hail <- model_data %>% 
  model.frame(report ~ mesh + mesh_west + folds, data = .)
model_matrix_hail <- model.matrix(report ~ mesh + mesh_west + folds, data = model_frame_hail)
