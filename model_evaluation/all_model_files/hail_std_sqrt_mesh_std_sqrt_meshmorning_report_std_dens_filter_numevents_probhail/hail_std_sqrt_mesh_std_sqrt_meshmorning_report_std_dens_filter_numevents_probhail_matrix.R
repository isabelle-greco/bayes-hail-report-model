...
model_data <- read_csv(args[2], col_types = "nnTnnnnnnncnffffffn") %>%
  mutate(mesh_morning = replace_na(mesh_morning, 0)) # na's to zero


# create matrices
# matrix for hail
model_frame_hail <- model_data %>% 
  model.frame(report ~ sqrt(mesh) + sqrt(mesh_morning) + folds, data = .)
model_matrix_hail <- model.matrix(report ~ sqrt(mesh) + sqrt(mesh_morning) + folds, data = model_frame_hail)
