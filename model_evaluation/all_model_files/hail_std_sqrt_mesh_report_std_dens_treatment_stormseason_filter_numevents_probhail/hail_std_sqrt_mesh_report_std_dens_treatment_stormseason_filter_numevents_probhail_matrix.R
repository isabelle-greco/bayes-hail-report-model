...
model_data <- read_csv(args[2], col_types = "nnTnnnnnnncnffffffn") %>%
  # storm_season  as an ordered factor
...
model_frame_hail <- model_data %>% 
  model.frame(report ~ sqrt(mesh) + folds, data = .)
model_matrix_hail <- model.matrix(report ~ sqrt(mesh) + folds, data = model_frame_hail)
...
model_frame_report <- model_data %>% 
  ...
  mutate(report = case_when(report == 0 ~ 0,
                            report == 1 ~ 1)) %>%  
  model.frame(~ log(pop_dens) + storm_season + report + folds, data = .)
model_matrix_report <- model.matrix(~ log(pop_dens) + storm_season + report + folds, data = model_frame_report,
...
beta_report_lower_bounds <- c(-Inf, 0.0, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf)
