...
// transformed data 
transformed data {
  // proportion for hail days 
  real<lower=0, upper=1> prop_data = N_train == N_test ? 1.0 : 1.0 * N_train / (N_train + N_test);
}
// parameters
parameters {
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[3] | 1.0, 1.0 / 3.0);
  target += normal_lpdf(beta_report_raw[1] | -2.0, 8.0);
  target += normal_lpdf(beta_report_raw[2] | 0.5, 1.0);
  target += normal_lpdf(beta_report_raw[3] | 1.0, 1.0 / 3.0);
  // filter with information about expected total hail events in training set
  target += normal_lpdf(expected_total_hail_events_train | 1490.0 * prop_data, 420.0 * prop_data);
}
