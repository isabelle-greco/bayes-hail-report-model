### Script to create the relevant matrices for this model.
###
...
model_data <- read_csv(args[2], col_types = "nnTnnnnnnncnffffffn") %>%
  mutate(across(starts_with("mesh_"), \(x) replace_na(x, 0))) %>% # replacing NAs in mesh_ vars
  # making mesh_max variable from max of adjacent mesh
  rowwise() %>%
  mutate(mesh_max = max(c_across(c(starts_with("mesh_"), -mesh_morning)))) %>%
  ungroup()
...
model_frame_hail <- model_data %>% 
  model.frame(report ~ mesh + mesh_max + folds, data = .)
model_matrix_hail <- model.matrix(report ~ mesh + mesh_max + folds, data = model_frame_hail)
...
model_frame_report <- model_data %>% 
  ...
  mutate(report = case_when(report == 0 ~ 0,
                            report == 1 ~ 1)) %>%  
  model.frame(~ pop_dens + report + folds, data = .)
model_matrix_report <- model.matrix(~ pop_dens + report + folds, data = model_frame_report)
...
beta_hail_lower_bounds <- c(-Inf, 0.0, -Inf, -Inf, -Inf)
beta_report_lower_bounds <- c(-Inf, 0.0, -Inf)
