...
data {
  ...
  vector[4] beta_hail_lower_bounds;
...
// parameters
parameters {
  vector<lower=beta_hail_lower_bounds>[4] beta_hail_raw;
...
transformed parameters {
  ...
  // applying transform
  for (i in 1:N_train) {
    mesh_train_yj_hail[i] = yeo_johnson(X_mesh_train[i, 3], beta_hail_raw[4]);
  }
  for (i in 1:N_test) {
    mesh_test_yj_hail[i] = yeo_johnson(X_mesh_test[i, 3], beta_hail_raw[4]);
  }
  ...
  matrix[N_train, 3] X_mesh_train_std_trans = append_col(X_mesh_train_std, (mesh_train_yj_hail - mesh_train_yj_hail_mean) / mesh_train_yj_hail_sd);
  matrix[N_test, 3] X_mesh_test_std_trans = append_col(X_mesh_test_std, (mesh_test_yj_hail - mesh_train_yj_hail_mean) / mesh_train_yj_hail_sd);
  
  {
    // can't have constraints in these local blocks
    // note the yj parameter is in the last entry of beta_hail
    vector[N_train] prob_hail_train = inv_logit(X_mesh_train_std_trans * beta_hail_raw[1:3]);
  ...
  beta_hail[1] = beta_hail_raw[1] - (beta_hail_raw[2] * (mesh_train_mean / mesh_train_sd)) - (beta_hail_raw[3] * (mesh_train_yj_hail_mean / mesh_train_yj_hail_sd));
  beta_hail[2] = beta_hail_raw[2] / mesh_train_sd; 
  beta_hail[3] = beta_hail_raw[3] / mesh_train_yj_hail_sd;
  beta_hail[4] = beta_hail_raw[4];
}
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[3] | 0.0, 1.0);
  target += normal_lpdf(beta_hail_raw[4] | 1.0, 2.0 / 3.0);
...
generated quantities {
  ...
  // probability of report 
  {
    vector[N_test] prob_hail_test = inv_logit(X_mesh_test_std_trans * beta_hail_raw[1:3]);
