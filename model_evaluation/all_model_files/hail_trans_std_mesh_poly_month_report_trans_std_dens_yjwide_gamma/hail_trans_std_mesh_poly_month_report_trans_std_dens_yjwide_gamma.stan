...
data {
  int<lower=0> N_train;
  int<lower=0> N_test;
  matrix[N_train, 9] X_mesh_train;
  matrix[N_test, 9] X_mesh_test;
  ...
  vector[10] beta_hail_lower_bounds;
  vector[3] beta_report_lower_bounds;
}
// parameters
parameters {
  vector<lower=beta_hail_lower_bounds>[10] beta_hail_raw;
...
transformed parameters {
  ...
  matrix[N_train, 9] X_mesh_train_std_trans;
  matrix[N_test, 9] X_mesh_test_std_trans;
  ...
  X_report_test_std_trans[, 2] = (dens_test_yj - dens_train_yj_mean) / dens_train_yj_sd;
  {
    ...
    // note the yj parameter is in the last entry of beta_report
    vector[9] beta_hail_raw_mult = append_row(beta_hail_raw[1:2], beta_hail_raw[4:10]);
  ...
  beta_hail[3:10] = beta_hail_raw[3:10];
}
...
model {
  ...
  target += normal_lpdf(beta_hail_raw[4:10] | 0.0, 1.0 / 3.0);
